...
# Ultimate-Coupon-Feed paths. With trailing slash.
...
class UCFeed {               
  ...
  var $sections = array('list','setup', 'edit','reset', 'logs', 'testfeed', 'forcefetch');  
  ...
  {              
    ...
    $this->db = array(
      ...
      'campaign'            => $wpdb->prefix . 'ucf_campaign',

      'campaign_category'   => $wpdb->prefix . 'ucf_campaign_category',

      'campaign_feed'       => $wpdb->prefix . 'ucf_campaign_feed',     

      'campaign_word'       => $wpdb->prefix . 'ucf_campaign_word',   

      'campaign_post'       => $wpdb->prefix . 'ucf_campaign_post',

      'log'                 => $wpdb->prefix . 'ucf_log'
    ...
    $this->installed = get_option('ucf_version');

    $this->setup = get_option('ucf_setup');
    ...
    # Ultimate-Coupon-Feed URIs. Without trailing slash               

    $this->optionsurl = get_option('siteurl') . '/wp-admin/options-general.php';                                           

    $this->adminurl = $this->optionsurl . '?page=ucfeed.php';

    $this->pluginpath = get_option('siteurl') . '/wp-content/plugins/Ultimate-Coupon-Feed';           

    $this->helpurl = $this->pluginpath . '/help.php?item=';

    $this->tplpath = $this->pluginpath . '/inc/admin';

    $this->cachepath = WPODIR . get_option('ucf_cachepath');

    

    # Cron command / url

    $this->cron_url = $this->pluginpath . '/cron.php?code=' . get_option('ucf_croncode');
...

  {
    ...
    WPOTools::addMissingOptions(array(
     ...
     'ucf_log'          => array(1, 'Log Ultimate-Coupon-Feed actions'),

     'ucf_log_stdout'   => array(0, 'Output logs to browser while a campaign is being processed'),

     'ucf_unixcron'     => array(WPOTools::isUnix(), 'Use unix-style cron'),

     'ucf_croncode'     => array(substr(md5(time()), 0, 8), 'Cron job password.'),

     'ucf_cacheimages'  => array(0, 'Cache all images. Overrides campaign options'),

     'ucf_cachepath'    => array('cache', 'Cache path relative to ucfeed directory')
    ...
    {			
			...
			dbDelta( "CREATE TABLE {$this->db['campaign']} (
							    ...
							    posttype enum('publish','draft','private') NOT NULL default 'draft',
    ...
    dbDelta(  "CREATE TABLE {$this->db['campaign_post']} (
     ...
     dbDelta(  "CREATE TABLE {$this->db['log']} (
      ...
      add_option('ucf_version', $this->version, 'Installed version log');


	 $wpdb->query("INSERT INTO {$this->db['campaign']} (id, title, active, slug, template, frequency, feeddate, cacheimages, posttype, authorid, comment_status, allowpings, dopingbacks, max, linktosource, count, lastactive, created_on) VALUES

(1, 'Albertsons', 1, 'albertsons', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(2, 'CVS', 1, 'cvs', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(3, 'Online Deals', 1, 'online-deals', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(4, 'Restaurant Deals', 1, 'restaurant-deals', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(5, 'Target', 1, 'target', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(6, 'Walgreens', 1, 'walgreens', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00'),

(7, 'Walmart', 1, 'walmart', '', 104400, 0, 0, 'draft', 1, 'open', 1, 0, 10, 0, 0, '0000-00-00 00:00:00', '0000-00-00 00:00:00');" ); 

	 	      $wpdb->query(  "INSERT INTO {$this->db['campaign_category']} (id, category_id, campaign_id) VALUES
...
(32, 1, 7);" ); 
	  ...
	  $wpdb->query("INSERT INTO {$this->db['campaign_feed']} (id, campaign_id, url, type, title, description, logo, count, hash, lastactive) VALUES

(1, 1, 'http://www.homebudgetideas.com/topics/albertsons-deals/feed', '', 'Home Budget Ideas.com » Albertsons Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:47:32'),

(2, 2, 'http://www.homebudgetideas.com/topics/cvs-deals/feed', '', 'Home Budget Ideas.com » CVS Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:48:36'),

(3, 3, 'http://www.homebudgetideas.com/topics/online-deals/feed/', '', 'Home Budget Ideas.com » Online Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 8, '', '2011-09-01 04:48:34'),

(4, 4, 'http://www.homebudgetideas.com/topics/restaurant-deals/feed', '', 'Home Budget Ideas.com » Restaurant Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:48:38'),

(5, 5, 'http://www.homebudgetideas.com/topics/target-deals/feed', '', 'Home Budget Ideas.com » Target Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:48:39'),

(6, 6, 'http://www.homebudgetideas.com/topics/walgreens-deals/feed', '', 'Home Budget Ideas.com » Walgreens Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:48:40'),

(7, 7, 'http://www.homebudgetideas.com/topics/walmart-deals/feed', '', 'Home Budget Ideas.com » Walmart Deals', 'Extreme Coupons, Budgeting Ideas &amp; Tips', '', 10, '', '2011-09-01 04:48:40');" ); 
  ...
  {   
    ...
    WPOTools::deleteOptions(array('ucf_log', 'ucf_log_stdout', 'ucf_unixcron', 'ucf_croncode', 'ucf_cacheimages', 'ucf_cachepath'));
   ...
   * Checks that Ultimate-Coupon-Feed tables exist
...

  {
    ...
    if(get_option('ucf_log_stdout'))

      echo $message;    

    

    if(get_option('ucf_log'))
...

  {
...

    {
    	...
    	$campaignid = (int) get_post_meta(get_the_ID(), 'ucf_campaignid', true);
...

    	{
    	  ...
    	  if($campaign->linktosource)
    	    ...
    	    return get_post_meta(get_the_ID(), 'ucf_sourcepermalink', true);
...

  {
...

    {
      ...
      $this->processItem($campaign, $feed, $item,$i_no);
  ...
  function processItem(&$campaign, &$feed, &$item, &$i_no)

  {
    ...
    $meta = array(
      ...
      'ucf_campaignid' => $campaign->id,

      'ucf_feedid' => $feed->id,

      'ucf_sourcepermalink' => $item->get_permalink()
  ...
  {  
    ...
    if(get_option('ucf_cacheimages') || $campaign->cacheimages)
...

  {
    ...
    { 
      ...
      return $this->pluginpath . '/' . get_option('ucf_cachepath') . '/' . $filename;
  ...
  function fetchFeed($url, $stupidly_fast = true, $max = 0)
   ...
   * Check If Post is generated by UCFEED
   ...
   */
  function checkCampaignPosts($id)

  {

    global $wpdb;

    return $wpdb->get_results("SELECT post_id FROM {$this->db['campaign_post']} WHERE post_id = $id ");          
   ...
   * Called when Ultimate-Coupon-Feed admin pages initialize.
...
<div id='wpo-warning' class='updated fade-ff0000'>

  <p>" . sprintf(__('Please <a href="%s">click here</a> to setup and configure Ultimate-Coupon-Feed.', 'ucfeed'), $this->adminurl . '&amp;s=setup') . "</p>
...
"; 
...

  {
    ...
    add_submenu_page('options-general.php', 'Ultimate-Coupon-Feed', 'Ultimate-Coupon-Feed', 10, basename(__FILE__), array(&$this, 'admin'));
...

  {
      ...
      update_option('ucf_unixcron', isset($_REQUEST['option_unixcron']));

      update_option('ucf_setup', 1);
  ...
  {                
...

    {
      ...
      check_admin_referer('ucfeed-edit-campaign');
  	...
  	if ( $categories ) {
  		...
  		foreach ( $categories as $category ) {
  			...
  			if ( $category->parent == $parent) {
  				...
  				echo "\t" . _ucf_edit_cat_row($category, $level, $data);
  ...
  {  
    ...
    {              
      ...
      update_option('ucf_log',          isset($_REQUEST['option_logging']));

      update_option('ucf_log_stdout',   isset($_REQUEST['option_logging_stdout']));      

      update_option('ucf_cacheimages',  isset($_REQUEST['option_caching']));

      update_option('ucf_cachepath',    rtrim($_REQUEST['option_cachepath'], '/'));
...

  {
...

    {
      ...
      if(!isset($_REQUEST['feed']))
        ...
        $add_error = __('You must select at least one feed', 'ucfeed');
...

      {
...

        {
          ...
          case '1':
            ...
            $this->add_success = __('Campaigns added successfully. Feel free to edit them', 'ucfeed');
          ...
          case '2':
            ...
            $this->add_success = sprintf(__('Feeds added successfully. <a href="%s">Edit campaign</a>', 'ucfeed'), $this->adminurl . '&s=edit&id=' . $campaignid);
...

  {
...

    {
...

      {
        ...
        $this->export_error = __('Please select at least one campaign', 'ucfeed');
...

      {
        ...
        header('Content-Disposition: attachment; filename="ucfeed.opml"');
  ...
  {  
...

    {
      ...
      $errors['basic'][] = __('You have to enter a campaign title', 'ucfeed');
...

    {
      ...
      $errors['categories'][] = __('Select at least one category', 'ucfeed');
...

      {
...

        {
...

          {
            ...
            $errors['rewrites'][] = __('There\'s an error with the supplied RegEx expression', 'ucfeed');         
...

    {
      ...
      $errors['options'][] = __('Author username not found', 'ucfeed');
...

    {
      ...
      $errors['options'][] = __('Selected frequency is not valid', 'ucfeed');
...

    {
      ...
      $errors['options'][] = __('Max items should be a valid number (greater than zero)', 'ucfeed');
...

    {
      ...
      $errors['options'][] = sprintf(__('Cache path (in <a href="%s">Options</a>) must be writable before enabling image caching.', 'ucfeed'), $this->adminurl . '&s=options' );
...

  {
    ...
    {      
      ...
      $this->tool_success = __('All posts removed', 'ucfeed');
...

    {
      ...
      $this->tool_success = __('Posts status updated', 'ucfeed');
...

    {
...

      {
        ...
        $this->errors = array('tools' => array(sprintf(__('Author %s not found', 'ucfeed'), attribute_escape($_REQUEST['campaign_tool_changeauthor']))));
...
function like_content($content) {
	...
	$cl = new UCFeed();
	$pid = get_the_id();
	if($cl->checkCampaignPosts($pid)){
		$contentHtml = '<p>Thanks to HomeBudgetIdeas.com for this <a href="http://www.homebudgetideas.com">Grocery Coupon</a></p>';
